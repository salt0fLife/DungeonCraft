...
const sound_lookup = {
	"stone_s" : ["res://assets/sounds/footsteps/stone/footstepStone1.wav",
	...
	"water_s" : ["res://assets/sounds/footsteps/water/footstepConcrete2.wav",
...
@onready var effects_lookup = {
	"water_s" : preload("res://assets/effects/small_water_splash.tscn"),
	
}

func play(pos : Vector3 = global_position, rot : float = global_rotation.y):
	var k = "stone_s"
	if $groundFinder.is_colliding():
		var g = $groundFinder.get_collider().get_groups()
		if !g.is_empty():
			if sound_lookup.has(g[0]):
				k = g[0]
			if effects_lookup.has(g[0]):
				var effect = effects_lookup[g[0]].instantiate()
				effect.position = pos
				effect.rotation.y = rot
				$effect_handler.add_child(effect)
			#match g[0]:
				#"stone_s" : k = stone
