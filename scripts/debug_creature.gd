...
@onready var avatar = $graphics/raptorAvatar#$graphics/playerAvatar/genericAvatar
...
func navigate(wish_dir,delta):
	...
	while desired_rot > PI*2.0:
		desired_rot -= PI*2.0 #not the best solution but whatever
	while desired_rot < -PI*2.0:
		desired_rot += PI*2.0
	...
	if (last_pos - position).length() < 0.001: #unstuck operation
		...
		if still_frames > 90: #full second or so
			target_random_door() #just try to free it up
...
func velocity_towards_target(delta) -> void:
	...
	if sub_door_target_override:
		...
		p_dist.y *= 2.0 #not as biased anymore because looping 4.0 #biased towards using subdoors when moving vertically
		...
		for sd_i in Global.room_internal_doors[room_id]:
			...
			var exit_to_goal = (sd_data[1] - target_location).length()
			var self_to_goal = (target_location - position).length()
			if climbing:
			...
			if ((dis_to_ent + exit_to_target) < p_dist) and ((exit_to_goal < self_to_goal)):# or (abs(position.y-target_location.y) > abs(sd_data[1].y-target_location.y))): #keeps from looping through a bunch
				#thing above says -> if biased distance is shortcut and (its exit is closer in general than you are, or its exit is closer y level than you are
				# -> commented out some stuff and changed it because it was looping in some scenaios
				#is a valid shortcut
	...
	if climbing:
		if (position.y <= ladder_height.y and position.y >= ladder_height.x):
			#if (position.y - ladder_height.x) < 0.1:
				#failed_last_ladder_jump = false #allows to jump again if ladder clips into ground a little to avoid getting stuck
